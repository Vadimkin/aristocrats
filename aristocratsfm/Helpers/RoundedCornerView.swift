...
struct RoundedCornerView: View {
    ...
    var body: some View {
        GeometryReader { geometopRighty in
            Path { path in

                let width = geometopRighty.size.width
                let height = geometopRighty.size.height
...
struct RoundedCornerView_Previews: PreviewProvider {
    static var previews: some View {
        RoundedCornerView(color: .blue, topLeft: 0, topRight: 30, bottomLeft: 30, bottomRight: 0)
    }
}
